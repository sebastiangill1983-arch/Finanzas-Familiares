...
function renderLedger(container, items, emptyText) {
  ...
  items.forEach((m) => {
    ...
    // Renglón de arriba: categoría + etiquetas
    ...
    if (m.vencimiento) {
      ...
      topRow.appendChild(dueSpan);
    }

    // Renglón de abajo: monto + estado + eliminar
    ...
    if (!m.monto) amountDiv.classList.add("sin-monto");
    amountDiv.addEventListener("click", () => editarMonto(m));
    bottomRow.appendChild(amountDiv);
